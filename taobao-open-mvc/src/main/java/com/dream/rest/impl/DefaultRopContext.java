...
import com.dream.rest.ServiceMethodDefinition;
...
public class DefaultRopContext implements RopContext {
    ...
    private void registerFromContext(final ApplicationContext context) throws BeansException {
        ...
        if (logger.isInfoEnabled()) {
            logger.info("共注册了" + serviceHandlerMap.size() + "个服务方法，方法列表如下所示：");
        }
        
        if(logger.isInfoEnabled()){
        	logger.info("*===============================================================*");
        	for(Map.Entry<String, ServiceMethodHandler> e :serviceHandlerMap.entrySet()){
        		logger.info("++"+e.getKey()+"="+e.getValue().getServiceMethodDefinition().getMethod());
        	}
        	logger.info("*===============================================================*");
        }
