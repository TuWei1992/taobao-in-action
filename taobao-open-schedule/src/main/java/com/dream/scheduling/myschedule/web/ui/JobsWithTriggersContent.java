...
//    private final TaskService taskService = (TaskService) SpringContextLoaderListener.getSpringWebApplicationContext().getBean("taskService");
//    private final AddJobService addJobService = (AddJobService)SpringContextLoaderListener.getSpringWebApplicationContext().getBean("addJobService");
    ...
    private void initToolbar() {
        ...
        tableRowActionButtonsGroup.addComponent(reloadTimeWithServer());
    ...
    private Button reloadTimeWithServer() {
    	 Button button = new Button("Reload Time");
         button.addClickListener(new Button.ClickListener() {
             @Override
             public void buttonClick(Button.ClickEvent event) {
                 ConfirmDialog.show(myScheduleUi, "Are you sure to run it now?",
                         new ConfirmDialog.Listener() {
                             @Override
 							public void onClose(ConfirmDialog dialog) {
                             	//更新B2C_TASK表中的任务状态置为0
//                             	try {
//// 									taskService.updateScheduleStatus();
// 								} catch (tabao-openServiceException e) {
//// 									e.printStackTrace();
// 								}
                             	//查询未调度的任务，即任务状态为0的任务
                                 if (dialog.isConfirmed()) {
                                 ...
                                 //同步jobDetailManagerTrigger下次执行时间
                                 try {
// 									addJobService.scheduleJob("jobDetailManagerJobDetail","jobDetailManagerJobDetail", null, "jobDetailManagerTrigger", "15 * * * * ?",  "DEFAULT");
                                 } catch (Exception e) {
// 									e.printStackTrace();
 								}
                             }
