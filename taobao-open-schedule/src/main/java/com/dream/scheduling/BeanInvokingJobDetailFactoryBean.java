...
import com.dream.scheduling.model.JobContextContainer;
...
public class BeanInvokingJobDetailFactoryBean implements FactoryBean, BeanNameAware, InitializingBean, ApplicationContextAware {
	...
	public void afterPropertiesSet() throws Exception {
		try {
			...
			jobDetail.getJobDataMap().put("amcOnOff", amcOnOff);
	 ...
	 * add by wulei
	 */
	private boolean amcOnOff = false;
	
	public boolean isAmcOnOff() {
		return amcOnOff;
	}

	public void setAmcOnOff(boolean amcOnOff) {
		this.amcOnOff = amcOnOff;
	}
		...
		public void execute(JobExecutionContext context) throws JobExecutionException {
			try {
				...
				if(null == applicationContext) {
					logger.info("com.icbc.tabao-open.scheduling.BeanInvokingJobDetailFactoryBean$BeanInvokingJob#execute : ApplicationContext is null !");
					return;
				}
				...
				//获取应用监控开关
				boolean amcOnOff = context.getMergedJobDataMap().getBoolean("amcOnOff");
				logger.debug("amcOnOff is " + amcOnOff);
				
				//获取jobDetailName播放TaskLogEvent事件
				String jobDetileName = context.getJobDetail().getName();
				JobContextContainer.setJobDetileName(jobDetileName);
				JobContextContainer.setAmcOnOff(amcOnOff);
//				applicationContext.publishEvent(new TaskLogEvent(this, jobDetileName, amcOnOff));
				...
				beanMethod.invoke();
				logger.info("Invoking Bean: " + targetBean + "; Method: " + targetMethod  + "is over load!");
